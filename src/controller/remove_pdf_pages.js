...
// PDF Pages Delete endpoint
exports.DeletePdfPages = async (req, res) => {
  ...
  getClientIdAndProcess(req, res, async (clientId) => {
    ...
    try {
      ...
      console.log(`Parsing page ranges to delete: "${pageRangesStr}"`);
      ...
      if (pageNumbers.length === pageCount && hasValidRanges) {
        return res
          .status(400)
          .json({ error: "Cannot delete all pages from the PDF" });
      }

      // Delete specified pages (in descending order to avoid index shifting)
      const pagesToDelete = pageNumbers.map((num) => num - 1); // Convert to 0-based indices
      pagesToDelete.sort((a, b) => b - a); // Sort in descending order
      for (const index of pagesToDelete) {
        inputPdfDoc.removePage(index);
      }
      ...
      const outputPdfBytes = await inputPdfDoc.save();
      fs.writeFileSync(outputFilePath, outputPdfBytes);
      console.log(`Saved modified PDF at ${outputFilePath}`);
      ...
      res
        .status(400)
        .json({ error: `Failed to delete PDF pages: ${err.message}` });
    }
  });
};
